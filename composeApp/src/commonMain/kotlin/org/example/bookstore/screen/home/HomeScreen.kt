...
fun HomeScreen(
    ...
    } else if (isSupplierDialogOpened.value && selectedEntity.value == Entity.SUPPLIER) {
        SupplierDialog(
            supplierDialogState = supplierDialogState.value,
            onStateChanged = { viewModel.onSupplierDialogChanged(it) },
            onConfirmRequest = { viewModel.onDialogConfirmed() },
            onDismissRequest = { viewModel.onDialogRejected() }
        )
    }
        ...
        trailing = {
            Box {
                EntityMoreActionsMenu(
                    dropdownMenuState = dropdownMenuState.value,
                    onEditEntity = {
                        viewModel.onEditSupplierRequest(supplier = supplier)
                        dropdownMenuState.value.status = DropdownMenuState.Status.Closed
                    },
                    onDeleteEntity = {
                        viewModel.onDeleteSupplierRequest(supplierId = supplier.supplierId!!)
                        dropdownMenuState.value.status = DropdownMenuState.Status.Closed
                    }
                )
                IconButton(
